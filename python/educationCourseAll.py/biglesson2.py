...
# dogy = {name:'Ronny', ade:8} # например словарь дог
#
...
# - по ключам
for key in friend.keys(): # мы будем идти по циклу, печатать ключи, пок ане переберём все ключи. через точку вызываем метод keys
...
# но мы можем сразу получить в одну переменную ключь, во вторую значение
...
# какие вещи повтаряются?
print(max_things & kate_things)
# воспользуемся пересечением & . знак амперсант - &
