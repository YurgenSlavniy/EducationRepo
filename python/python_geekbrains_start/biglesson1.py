...
#
# Запустим пайчарм первый раз. предлагает сначала импортировать настройки.
# Соглашаемся с лицензионным использованием. Будем отправлять статистику.
# Далее предлагается выбрать тему тёмную или светлую.
# Плагины оставим пок абез изменения.
# Криейт нью проджект. (create new project)
# давайте напишем имя проекта. и нажимаем создать!
# можно изменить папку по умолчанию, путь куда сохранить файл.
# далее при запуске пайчарм будет настраивать окружение.
# после того как он это сделает мы сможем писать код.
# последующие разы когда мы будем запускать пайчарм он уже не будет это делать.
# все будет происходить гораздо быстрее
# перехожу далее в пайчарм и буду писать уже там. этот текст копирую туда же.
#
# Что мы видим, открыв пайчарм? :
# слева (лп) у нас есть структура проекта.
# Пайчарм создал для нас виртуальное окружение (venv library root) в котором мы можем работать.
# давайте создадим теперь в папке educationCourseAll.py  питон файл.
# пкм в лп по названию папки во всплывающем меню - new  -->  python file
# файлы у нас зоздаются с расширением py?
# именно эти файлы понимает интерпритатор, который мы установили.
# давайте напишем нашу первую программу хеллоу ворлд:
# файл - новый - pr1helloworld.py
#
...
# запускаем заново и видим у нас есть  строчка  - first you read this
#
#______________________________
# УРОК 2 -  ПЕРЕМЕННЫЕ ТИПЫ ДАННЫХ ПРЕОБРАЗОВАНИЕ ТИПОВ.
#______________________________
#
# Переменные используют для хранения данных.
...
pty = 'значение'
# Объявим три переменные : имя попугайчика присваиваем значение кеша.
# возраст попугайчика присваиваем значение 2
# и период через который научился говорить пристваиваем 2.
name4 = 'Олег'
age = 1
period = 5
...
#
# В программировании очень важно правильно называть переменные.
# Чтобы по её значению можно было понять для чего она нужна.
...
is_good = True
person = None
# Каждая переменная обладает определённым типом:
# тип переменной определяет множество значений, которые могут быть ей присвоены
# и операции которые могут быть с нею произведены.
# Он либо фиксирован в момент объявления переменной
# и соответствует одному из типов данных предоставляемых
# языком программирования (статическая типизация).
# либо в каждый момент соотвветствует типу тех данных,
# что содержит переменная (динамическая типизация)
# динамическая типизация:
# как только мы присваиваем значение переменной
# мы автоматически присваиваем ей соответствующий тип.
# целое число - int
...
# пишем функцию type(person_name1) в скобках указываем переменную у которой мы хотим определить тип,
# с помощью принт всё это выводим на экран чтобы посмотреть результат
...
# мы прочитали из файла или ввёл пользователь год рождения и получилась
# у нас вместа числа строка. т.к записано в ковычках - это строчка.
period2 = 30
print(type(period2))
...
print(name3, animal, age4)
# просто указываем переменные через запятую.
# print возьмёт эти переменные и сам сделает из них строчку
...
#    действие 1
#    действие 2
name5 = input('КТО создатель пайтона ?')
...
# - вывести числа от 0 до 18
# - вывести числа от 0 до n, где n вводит пользователь
# - вывод чётных чисел от 0 до n
#
# вывести числа от 0 до 100
number = 0 # нам потребуется переменная number2 которая изначально равна нулю

while number <= 18:  # мы сравниваем переменную нумбер2 с 100, и пока оно меньше делаем вложенные в цикл оманды
    print(number) # печатаем значение переменной нумбер2
    number = number + 1 # прибавляем к этому значению 1 и идём снова на сравнение со 100.
#
number = 0

while number <= 7:
    print(number)
    number += 1 #  альтернатива написанию number2 = number2 + 1. берём переменную, прибавляем единичку и в эту же переменную записываем новое значение.
...
while number <= n:
    if number % 2 == 0: # если число в переменной нумбер2 чётное - т.е делится на 2 без остатка
        print(number) # условие чётности выполняется - выводим на экран, не выполняется - прибавляем единичку
    number += 1
...
name = input('Кто создатель пайтона?')
while name != 'Гвидо':
    print(' лезь и гугли! вспоминай не раз уже писал!ола!')
...
# если условие иф которое запросили уже внутри вайл будет верным, мы выходим из цикла с помощью брейк
# Скажем, что ответ не верный
...
while True:
    name7 = input('КТО разработал язык PYTHON?')
    if name7 == 'Гвидо':
        break
    print('Ну почему опять неверно!?')
...
print('end')
print("!!!ПОЗДРАВЛЯЮ ВЫ ЗАКОНЧИЛИ BIGLESSON 1!!!")
