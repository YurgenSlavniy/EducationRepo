...
```
Команды при установке и настройке mysql
```
~$: sudo apt update   |  Обновить список пакетов на анашем сервере
~$: sudo apt install mysql-server  | Установка mysql-server
~$: sudo mysql_secure_installation  |  Запуск скрипта безопасности с настройками
mysql> SELECT user,authentication_string,plugin,host FROM mysql.user;  | проверьте, какой метод аутентификации используют ваши аккаунты пользователей MySQL 
mysql> ALTER USER 'root'@'localhost' IDENTIFIED WITH caching_sha2_password BY 'password';  |   команда заменит пароль root, устанавливает аутентификацию root user MySQL с помощью плагина caching_sha2_password​​
mysql> ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY 'password'; | устанавливает аутентификацию root user MySQL с помощью плагина mysql_native_password.
mysql> FLUSH PRIVILEGES; |  команда просит сервер перезагрузить предоставленные таблицы и ввести в действие изменения
~$: mysql -u root -p  |  Запустить mysql сервер
mysql> exit |  Выход из sql
mysql> CREATE USER 'sammy'@'localhost' IDENTIFIED BY 'password'; | Создайте нового пользователя и придумайте для него надежный пароль:
mysql> GRANT ALL PRIVILEGES ON *.* TO 'sammy'@'localhost' WITH GRANT OPTION;  | предоставьте вашему новому пользователю соответствующие права. Например, вы можете предоставить пользователю права доступа ко всем таблицам в базе данных, а также можете добавлять, изменять и удалять права пользователя с помощью этой команды
~$: systemctl status mysql.service | MySQL должна запускаться автоматически. Чтобы проверить это, проверьте ее статус
~$: sudo mysqladmin -p -u root version | эта команда позволяет подключиться к MySQL в качестве пользователя root (-u root), запросить пароль (-p) и обеспечить возврат версии.
