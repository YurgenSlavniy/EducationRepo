...
# сортировка и ограничение. Агрегация данных.
...
# Операторы - конструкции языка,
# которые производят преобразование данных
# данные над которыми осуществляются операции - операнды
...
# результат - таблица из одного столбца и одной строки
# название столбца совпадает с вычисляемым значением
...
# автоматически приведтся к числу
# Но если строка не может быть приведена к числу
...
# при делении на ноль получаем NULL
#   mysql> SELECT 9 / 0;
# +-------+
# | 9 / 0 |
# +-------+
...
# +-------+
# 1 row in set (0.00 sec)
# оператор действует следующим образом: 5 - 2 * 2 = 1
...
# Операторы сравнения дают результатом
# истину или ложь
...
# позволяет сравнивать со значение NULL
...
# сравнение с NULL всегда возвращает NULL
...
# конструкция IS NULL  - ровно ли проверяемое значение NULL или нет
# вставим в таблицу catalogs ещё пару значений
...
# Выражение можно сохранять в таблицах
# оператор create table допускает создание столбцов
# значение которых автоматически создаётся как
# арифметическое значение других столбцов
...
# +------+------+------+
# 3 rows in set (0.00 sec)
# по умолчанию эти значения не сохраняются на жёсткий диск,
# а каждый раз вычисляются заново
# чтобы сохронялся надо добавить ключевое слово STORED
...
# т.к БД хронят по миллиону записей,
# ситауция когда требуется изменить
# количество вводимых строк встречается очень часто
# ключевое слово WHERE после которого следует
# логическое усовие
...
# для выборки записей из определённого интервала
# существует специальный оператор BETWEEN
# возвращает TRUE если значение входит в диапозон
...
# Конструкция NOT BETWEEN - возвращает значения
# не попадающие в интервал

# извлечь записи удовлетваряющие списку
# например записи с идентификаторами 1, 2, 5
#   mysql> SELECT * FROM catalogs WHERE id IN (1, 2, 5);
# возаращает FALSE если значение не входит в список
# TRUE если такое значение в списке имеется
# если в списке значение NULL возвращается NULL

# существует противоположная конструкция NOT IN
# возвращает TRUE для элементов,
...
# можем использовать не только числовой тип данных
# условную выборку с усастием строк удобно
# производить с помощью оператора LIKE
# LIKE возвращает истину, если шаблон соответствует.
...
# для помещения в шаблон самих символов без их интерпритации
# необходимо экранировать их с помощью обратного слэша
...
# календарный столбец преобразуется к строке
#   mysql> SELECT * FROM users WHERE birthday_at LIKE '199%';

# ОПЕРАТОРЫ RLIKE и REGEXP
# поиск в соотвествии с регулярными выражениями
# это специализированный язык поиска подстрок в тексте
#   mysql> SELECT content RLIKE '[[:digit:]]*\\.[[:digit:]]{2}';
# Имеется шаблон в котором также присутствуют спецсимволы
# слева от оператора RLIKE распологаем строку
# в которой осуществляем поиск
# справа находится регулярное выражение
...
#
# часто необходимо привязать регулярное выражение к началу слова
# чтобы регулярное выражение грам соответствовало
# подстроке которая начинается с грам
# символ ^ - соотвествует началу строки
...
# применение символов ^ $ позволяет указать
# что регулярное выражение должно в точности соответствовать
# всей строке от начала до конца

# для задания альтернатив символ |
# выступает как логическое или
#   mysql> SELECT 'abc' RLIKE 'abc|абв', 'абв' RLIKE 'abc|абв';

# если спецсимвол должен быть частью строки,
# то используем двойной обратный слэш для экранирования \\
# для задания классов символов
# используются квадратные скобки []
# ограничивают поиск теми символами,
...
# в квадратных скобках допускается
...
# существуют специальные классы,
# например для только что рассмотренного диапазона
# существует класс digit, который соответствует числам
# ему не будут удовлетворять строковые значения
# классы задаются в двойных квадратных скобках
#   mysql> SELECT 7 RLIKE '[[:digit:]]';

#  для строковых символов специальный класс альфа

# квантификаторы для того чтобы распространить
# действие класса на несколько символов.
# указываются сразу после квадратных скобок
...
# создадим регулярное выражение для цены
# т.е для 342.34
# целая часть состоит из любого количества цифр,
# а дробная всегда состоит из 2ух
...
# запрос выдаёт результаты в порядке,
# в котором они хронятся в БД
# записи не всегда будут выводиться
# в порядке добавления
# при добавлении, апдейте,
# удалении порядок может нарушаться
#
#   mysql> SELECT * FROM catalogs ORDER BY name;
# сортируем по столбцу name.
# по умолчанию сортировка в прямом порядке,
# в случае со строками по алфавиту
# если хотим в обратном порядке
...
# Когда большой объём информации
# лучше разбить её на страницы
# и предоставлять пользователю порциями

# для этого используем ключевое слово LIMIT
# после ставим число выводимых значений
...
# Можно указывать с какой позиции выводить
# с помощью ключевого слова OFFSET
#    mysql> SELECT id, catalog_id, price, name FROM products LIMIT 2 OFFSET 4;

# Часто стоит задача вывода
# уникальных значений из таблицы
#   mysql> SELECT DISTINCT catalog_id FROM products ORDER BY catalog_id;

# извлечение всех значений столбца
# в том числе и повторяющихся
...
# подобрать подходящие условия при помощи
# select запроса
# а потом заменить SELECT на DELETE и UPDATE
...
# предопределёные функции - готовые функции
# которые предоставляет система управления БД
# функции характеризуются именем и аргументами
# которые перечисляются за именем в круглых скобках
# результат который возвращает функция
# подставляется в место вызова
#   mysql> SELECT NOW();
# +---------------------+
# | NOW()               |
# +---------------------+
...
# добавим значение и используем явную временную функцию
# для заполнения полей с датой
#   mysql> INSERT INTO users LALUES (NULL, 'Александр'. '1985-07-13', NOW(), NOW());
...
# функция DATE_FORMAT(аргумент1, аргумент2)
# для форматирования календарных типов
...
# %Y отвечает за извлечение года из календарного значения
# со всеми последовательнастями (%Y, %D)
# можем ознакомиться по документации

# например отформатируем день рождения пользователя
# в удобном формате
#   mysql> SELECT name, DATE_FORMAT(birthday_at, '%d.%m.%y') AS birthday_at FROM users;

# Преобразование двты и времени в UNIXSTAMP формат
# т.е количество секунд которое прошло с полуночи
# 1 января 1970 года по 2038 год
# т.к это целое число его можно быстро обрабатывать,
# индексировать и оно занимает меньше места
# функция UNIX_TIMESTAMP()
# обратное преобразование FROM_UNIXTIME()
#   mysql> SELECT
...
# вычислим текущий возраст пользователя по полю birthday_at
# преобразуем дату рождения и текущую дату в дни
# полученные дни делятся на 365.25
#   mysql> SELECT
#         -> name,
...
# узнать значения присвоиные столбцу,
# снабжённому AUTO_INCREMENT
...
# можно использовать JSON поля.
# символ $ как начало коллекции
#   mysql> CREATE TABLE distances (
#   -> id SERIAL PRIMARY KEY,
#   -> a JSON NOT NULL,
#   -> b JSON NOT NULL,
...
# для некоторых задач требуются тригонометрические функции
# вычислить площадь треугольника,
# если известны длины сторон и угол между ними
...
#  т.к SIN ожидает значение аргумента в радианах
#  с помощью функции RADIANS() переводим градусы в радианы
...
# результат вычислений можно округлить при помощи функции ROUND()
# поменяем значение столбца
# ROUND(выражение, число знаков после запятой)
#   mysql> ALTER TABLE triangles CHANGE square square DOUBLE AS (ROUND(a * b * SIN(RADIANS(angle)) / 2.0, 4));
# осуществляется математическое округление
...
# Существует большое количество функций, обслуживающих строки
# выбрать из таблицы первые несколько символов
...
# ЛОГИЧЕСКИЕ ФУНУЦИИ позволяют преобразовать результат
# в зависимости от выполнения того или иного условия
# выведим слово совершеннолетний или несовершеннолетний
# в зависимости от того достиг пользователь 18 лет или нет
...
# Если необходимо проверить больше условий
...
# таблица содержит только английские названия цветов
# если при выводе захотим изменить англ. названия русскими
#   mysql> SELECT
#       -> CASE
...
# mysql предоставляет различные вспомогательные функции
# INET_ATON() принимает ай пи адрес и представляет его в виде целого числа
...
# используя операцию % - остаток от деления
# можем разбивать таблицу на отдельные группы
...
# SUBSTRING(birthday_at, 1, 3) - отображаем символы с 1 по 3 для колонки birthday_at
# и преобразуем к строковому значению
#   mysql> SELECT id, name, SUBSTRING(birthday_at, 1, 3) AS decode FROM users;
# назначаем для вычисляемого поля псевдоним AS decode
# который сможем использовать в других ключевых словах
...
# позволяют работать с содержимым групп
# полученных при помощи GROUP BY
# например подсчитать количество записей внутри каждой из групп
...
# назначим функции COUNT(*) псевдоним total
# и отсортируем по этому псевдониму
#   mysql> SELECT
#       ->   COUNT(*) AS total,
#       ->   SUBSTRING(birthday_at, 1, 3) AS decode
#       -> FROM
...
# ORDER BY не может быть раньше GROUP BY
# также относится к ключевому слову LIMIT,
# которое должно распологаться
# после овсех оставльных ключевых слов
...
# функция GROUP_CONCAT() допускает задание разделителя
# с помощью SEPARATOR
#   mysql> SELECT
...
# можно отсортировать значения в рамках
# возвращаемой строки с помощью  ORDER BY
#   mysql> SELECT
...
# имеет ограничения GROUP_CONCAT -
# максимум 1000 извлекаемых значений
...
# можем узнать количество значений в таблице.
# в качестве аргумента функция принимает название столбца
...
# функции MIN() и MAX() возвращают минимальное
# и максимальное значение столбца
...
#  влияющие на GROUP BY
# - условие HAVING
# - Получение уникальных значений
# - функция ANY_VALUE()
# - конструкция WITH ROLLUP

# каждая строка в результирующем запросе
# с участием GROUP BY
# представляет собой отдельную группу
# агригационные функции предназначены
# для получения результатов каждой из групп
...
#       -> FROM users
#       -> GROUP BY decode;
# например требуется выбрать где
# количества записей больше или равно 2
#   mysql> SELECT COUNT(*) AS total,
#       -> SUBSTRING(birthday_at, 1, 3) AS decode
#       -> FROM users
#       -> GROUP BY decode
...
# не можем взять агригационное значение
# и подставить в условие WHERE
...
# сформируем запрос на извлечение данных
# которые пойдут в промежуточную таблицу
#   mysql> SELECT
#       ->   name, description, price, catalog_id
#       -> FROM
#       ->   products
#       -> GROUP BY
#       ->   name, description, price, catalog_id;
# у нас остаются только уникальные значения
# уникальности добились при помощи GROUP BY
# GROUP BY может принимать несколько столбцов
# чтобы записи попали в группу
# все значения столбцов должны совпадать

# создадим таблицу products_new,
# которая полностью совпадает с таблицей products
# и с помощью операции INSERT SELECT
# мы все данные из временной таблицы
# перенесём в новую таблицу
...
# извлечём года на которые приходится дата раждения
# случайного пользователя из этого года
# посмотрим табличку и сгруппируем
...
#       -> ORDER BY
#       ->  birthday_year;
#  мы использовали игригационную функцию MAX
#  чтобы обойти ограничения на вывод имени пользователя.
#  потомучто при исполнении с name произойдёт ошибка:
#    mysql> SCELECT
#       ->  name,
#       ->  YEAR(birthday_at) AS birthday_year
#       -> FROM
...
# ANY_VALUE () для вывода случайного значения
#    mysql> SCELECT
#       ->  ANY_VALUE(name),
#       ->  YEAR(birthday_at) AS birthday_year
#       -> FROM
...
# конструкция позволяет добавить ещё одну строку
# суммы значений всех предыдущих строк
#   mysql> SELECT
#       ->  SUBSTRING(birthday_at, 1, 3) AS decode,
#       ->  COUNT(*)
#       -> FROM
...
# которые родились в 198(0) в 199(0) и в 200(0) года
# можем добавить последнюю результирующую строку
# с количеством всех пользователей
#   mysql> SELECT
#       ->  SUBSTRING(birthday_at, 1, 3) AS decode,
#       ->  COUNT(*)
#       -> FROM
