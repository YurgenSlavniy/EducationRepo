...
# SELECT ЗАПРСЫ
# сервисы по генерации данных.
# http://filldb.info/
# нажимаем красную кнопку START HERE
# вводим в открывшееся поле DDL команды
# попробывать в режиме инкогнито.
# сайт работает очень нестабильно.
# в сервисе надо данные генерировать по порядку,
# т.к таблицы взаимодействуют
# начинать надо с таблицы
# у которой нет зависимостей от других,
# но от которой зависимы другие таблицы
# после генерации, делаем экспорт и исполняем скрипт в DBeaver
...
# таблицу usres можно заполнять командой INSERT:
...
# ЗАДАНИЕ 2.
# Вывести список имён (только firstname) пользователей
...
#  но как правило GROUP BY идёт с агригирующими функциями
#    SELECT firstname
#    FROM users
#    GROUP BY firstname
#    ORDER BY firstname;
...
# ЗАДАНИЕ 3.
# Написать скрипт
# отмечающий несовершеннолетних пользователей
...
# пишем update запрос.
# WHERE (birthday + INTERVAL 18 YEAR) > NOW()
# берём дату рождения прибавляем интервал 18 лет
# и если она окажется больше настоящей даты,
# то пользователь несовершеннолетний
...
# Посмотрим варианты добавления столбца
...
# mysql считает тип данных bool число
# в интервале от -128 до 128
# boolian считается псевдонимом для tinyint
# если мы добавим в эти поля значения отличные от 0 и 1
# то они сохронятся и тогда будет ошибка данных
# В поле с типом данных BIT мы не сможем
# сохранить иные значения кроме 0 и 1
# ENUM('true', 'false') - строковые константы
# они занимают больше места чем биты 0 и 1
# ENUM('0', '1') - также строка
...
# когда удалённые данные ещё где то хронятся
...
# вместо delete запроса делаем update запрос
# и все сообщения из будущего отмечаем как True - 1
#   UPDATE messages
#   SET is_deleted = 1
#   WHERE created_at > NOW();
...
# SELECT ЗАПРОСЫ
# запустил скрипт и установил Базу данных
# которую разбирают на уроке

# пользователь зарегестрировался,
# вошёл на свою страничку
# и первое что он видит страничка своего профиля
# т.е информация о себе.
# наша БД организована так,
# что данные о пользователе
# разбросаны по разным таблицам.
# Если бы всё было в одной таблице запрос был бы прост
...
# в таблице users есть только firstname и lastname
# а информация 'city', 'main_photo' в этой таблице нет
# 'city' у нас находится в таблице profiles
# в колонке hometown
# мы заранее позаботились о связях между таблицами
# связь между users и profiles с помощью внешнего ключа
# поле user_id в profiles ссылается на id в таблице users
...
# разбираемся с фото.
# в DBreaver смотрим ERD диаграмму связи табличек
# фото находится в поле filename в таблице media
# Мы же обращаемся к табличке users  из неё
# можем перейти по связи к табличке profiles
# А потом из profiles в media по существующей связи
# profiles (photo_id) связь с media(id)
# кликаем по связи в ERD диаграмме чтобы увидить связь
# photo_id ссылается на id в в таблице media
#   mysql> SELECT filename FROM media WHERE id = (
#       -> SELECT photo_id FROM profiles WHERE user_id =1);
# в WHERE приравниваем первичный ключ к внешнему
# сначала исполнится вложенный запрос
# (SELECT photo_id FROM profiles WHERE user_id =1)
...
# можно заменить 88 на прямое обращение к таблице users
# к колонке id : users.id
# во вложенных запросах мы делаем ссылку на внешнюю таблицу
# весь запрос стал скалерированным.
# внешняя таблица - users
#   mysql> SELECT
#       -> firstname,
#       -> lastname,
...
# этим запросом выбрали все данные
# конкретизируем запрос
#   mysql> SELECT *
#       -> FROM media
#       -> WHERE user_id = 88;
# теперь мы получили записи
# только пользователя 88,
# но не все из них фото
# мы заранее позаботились о поле media_type_id
# который указывает на тип записи (поста)
# нам нужны только фото. зайдём в таблику media_type
# фотографиям соответствует первый медиатип. с id = 1
# фильтруем запрос ещё раз
#   mysql> SELECT *
#       -> FROM media
#       -> WHERE user_id = 88
#       ->   AND media_type_id = 1;
# если бы захотели получить видеозаписи
# мы позоботились о медиатипе заранее
# и для видео media_type_id = 3
#   mysql> SELECT *
#       -> FROM media
#       -> WHERE user_id = 88
#       ->   AND media_type_id = 3;

# Если бы мы ранее не позаботились о media_type_id
# тогда бы мы могли ориентироваться
# на расширение поля filename файл
# должен заканчиваться .mp4 или .avi
#   mysql> SELECT *
#       -> FROM media
#       -> WHERE user_id = 88
#       ->   AND (filename LIKE '%.mp4'
#       ->   OR filename LIKE '%.avi');
# в скобках указываем потому что иначе
# OR будет цеплять другие id у которых найдёт .avi
# не только 88 user_id
...
# SUM - сумма
# в общем случае аггригирующие функции
# используются вместе с группировкой

# Рассмотрим пример где есть
# аггригирующие функции и группировка
# узнаем количество новостей каждого типа
#   mysql> SELECT *
#       -> FROM media;
# мы видим разные медиатипы
# группируем всё по типу медиаданных
#   mysql> SELECT
#       ->   COUNT(*),
#       ->   media_type_id
#       -> FROM media
...
# усложним запрос.
# нам нужны только те пользователи,
# у которых более 1 медиазаписи
...
#       -> WHERE cnt > 1
#       -> GROUP BY user_id;
# такой запрос также приводит к ошибке
# даже если придумаем псевдоним

# AS cnt - результат работы аггрегирующей функции
# аггригирующая функция вычисляется после группировки
...
# UNION - вертикальное объединение запросов
# нужно чтобы набор полей
# и количество типов данных
# совпадали в верхнем и нижнем запросе,
# объединёных union

# Выбираем новости друзей
# лента новостей - записи из таблицы media
# которые относятся к друзьям
#   mysql> SELECT * FROM media;
...
# проблема в том, что мы изначально не знаем список друзей
# всё что мы знаем - собственный айдишник,
# а всё остальное намм надо получить динамически
# в процессе запроса
# можем использовать предыдущий запрос,
# внутри этого запроса
# чтобы сперва исполнился вложенный запрос,
# который получит список айдишников
# подставить его в фильтрацию WHERE
#   mysql> SELECT *
#       -> FROM media
#       -> WHERE user_id IN (
...
#       ->   AND status = 'approved'
#       ->   ) OR user_id =1;
# новости отсортируем в хронологическом порядке
# по убыванию. самые свежие вначале.
# старенькие в конце ORDER BY create_at DESC;
#   mysql> SELECT *
#       -> FROM media
#       -> WHERE user_id IN (
#       ->   SELECT  initiator_user_id
...
# показываем всю таблицу сообщений
# нам нужны только записи касаемые конкретного пользователя
# там где он был отправителем
#   mysql> SELECT *
#       -> FROM messeges
...
#   mysql> SELECT * FROM messeges;
# вся таблица сообщений
# диалог между 1 и 2 это где 1 был отправитель, 2 был получатель
# и где 2 был отправитель 1 был получатель
#   mysql> SELECT *
#       -> FROM messeges
...
# нужно реализовать функционал непрочитанных сообщений
# идём в таблицу messeges и смотрим есть ли у нас поле,
# отвечающее за прочитанность и непрочитанность
# такого поля нет, надо его добавить
# можем это сделать через запрос
...
# появляется всплывающее окно где даём имя нашей колонки
# и настройки.
#  тип данных - BIT
#  Not Null - ставим галочку
#  по умолчанию (DEFAULT) - ставим 0 (все сообщения не прочитанные)
# жмём OK
# чтобы это появилось в базе мы должны нажать save
# save (правый нижний угол)
# открылось окно с запросом
...
# у нас есть много сообщений от 8 к 1
# теперь тобы сообщения стали прочитанные,
# нужно чтобы 1 зашла в диалог с 8
# тогда значение в колонке is_read изменится на 1
#   mysql> UPDATE messeges
#       -> SET is_read =1
#       -> WHERE from_user_id = 8
#       ->  AND to_user_id = 1;

# Выведим информацию о друзьях в определённом виде
# друзья пользователя с преобразованием пола
# и подсчётом возраста
# user id, gender, age
# пол хотим получать понятным рускоязычным словом
# мужской, женский. А возраст надо преобразовывать,
# т.к у нас только дата рождения
#   mysql> SELECT
#       ->   user_id, gender, age
#       -> FROM profiles;
...
# для вывода - мужской и женский
# можем организовать ветвление
# внутри select запроса
#   mysql> SELECT
#       ->   user_id,
#       ->   CASE(gender)
#       ->      WHEN 'f' THEN 'женский'
...
# осталось разобраться с возрастом birthday
# можем использовать встроенные функции
# TIMESTAMPDIFF() которая вычисляет разницу
# между двумя датами
# в определённых единицах измерения
#   mysql> SELECT
#       ->   user_id,
#       ->   CASE(gender)
#       ->      WHEN 'f' THEN 'женский'
